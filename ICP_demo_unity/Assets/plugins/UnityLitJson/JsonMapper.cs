...
public class JsonMapper {
	...
	private static readonly Dictionary<string, Type> customHintedType;

	public static void RegisterCustomHintedType(string name, Type type)
	{
		customHintedType[name] = type;
	}
	...
	static JsonMapper() {
		...
		customHintedType = new Dictionary<string, Type>();
	}
	...
	public static object ReadValue(Type instType, JsonReader reader) {
		reader.Read();
			...
			if (reader.Token == JsonToken.ObjectEnd) {
				...
				if (reader.TypeHinting && property == reader.HintTypeName) {
					...
					if ((string)reader.Value == reader.HintValueName) {
						if (customHintedType.Count > 0)
						{
							customHintedType.TryGetValue(typeName, out valueType);
						}
	...
	private static void RegisterBaseExporters() {
		...
		baseExportTable[typeof(Guid)] = delegate(object obj, JsonWriter writer) {
	...
	private static void RegisterBaseImporters() {
		...
		RegisterImporter(baseImportTable, typeof(string), typeof(Guid), delegate(object input) {
			return new Guid((string)input);
		});
	}
	...
	private static void WriteValue(object obj, JsonWriter writer, bool privateWriter, int depth) {
		...
		if (obj is IList) {
			...
			foreach (object elem in list) {
				...
				if (writer.TypeHinting && elem != null && elem.GetType() != elemType) {
					...
					// Custom change
					bool customHintedTypeFound = false;
					if (customHintedType.Count > 0)
					{
						foreach (var kvp in customHintedType)
						{
							if (kvp.Value == elem.GetType())
							{
								customHintedTypeFound = true;
		...
		foreach (string property in tdata.Properties.Keys) {
			...
			if (pdata.IsField) {
				...
				object[] defaultAttrs = info.GetCustomAttributes(typeof(DefaultValueAttribute), true).ToArray();
				if (defaultAttrs.Length > 0) {
					if (EqualityComparer<object>.Default.Equals(((DefaultValueAttribute)defaultAttrs[0]).Value, value))
					{
						continue;
					}
				}

				if (writer.ExcludeDefaults)
				{
					...
					else if (EqualityComparer<object>.Default.Equals(default, value))
					{
						continue;
					}
				}
