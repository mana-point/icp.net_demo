...
public class JsonReader {
	...
	private static string eStr = "e";
	private static string EStr = "E";
	private static IFormatProvider culture = new CultureInfo("en-US").NumberFormat;
	private void ProcessNumber(string number) {
		if (number.Contains(dotStr) ||
			number.Contains(eStr) ||
			number.Contains(EStr))
		{
			float real;
			if (float.TryParse (number, NumberStyles.Float, culture, out real)) {
